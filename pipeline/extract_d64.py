...
def detokenize_basic(prg_data):
    """Convert tokenized BASIC v2 to source code."""
    if len(prg_data) < 2:
        return ""
    lines = []
    ...
    while offset < len(prg_data) - 4:
        ...
        while i < len(line_data):
            ...
            if b == 0x22:
                ...
                token_word = BASIC_TOKENS[b]
                if token_word in ("+", "-", "*", "/", "^", ">", "=", "<"):
                    tokens.append(f" {token_word} ")
                elif token_word in ("THEN", "STEP", "TO", "GO"):
                    tokens.append(f" {token_word} ")
                else:
                    tokens.append(f"{token_word}")
                i += 1
                ...
                tokens.append("{SW lowercase}")
                i += 1
            elif b == 0x0F:
                tokens.append("{SW uppercase}")
                i += 1
            elif b == 0x8F:
                tokens.append("{COLOR}")
                i += 1
